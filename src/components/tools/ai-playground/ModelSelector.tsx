...
export function ModelSelector() {
  const { settings, updateSettings, cachedModels } = useAIPlaygroundStore();
  ...
  const availableModels = useMemo(() => {
    ...
    // Merge: cached models take priority but hardcoded ones add metadata
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {/* Model selector */}
      <DropdownMenu open={modelOpen} onOpenChange={(o) => { setModelOpen(o); if (!o) setModelSearch(""); }}>
        <DropdownMenuTrigger asChild>
          <Button variant="outline" size="sm" className="gap-1 text-xs h-8 max-w-[200px]">
            <span className="truncate">{currentModel?.name || settings.selectedModelId || "Select model"}</span>
