...
const CategoryCard = memo(
  ({
    ...
    >
      <a href={`/${category.toLowerCase().replace(" ", "-")}`} className="group block">
        <div
...
const ToolGrid = ({ initialCategory }: { initialCategory?: string }) => {
  const tools = availableTools;
  const { favorites, recentlyUsed, isFavorite } = useToolsStore();
  const favoriteTools = availableTools.filter(t => favorites.includes(t.id));
          ...
          <div className="grid lg:grid-cols-[1.2fr_0.8fr] gap-12 relative z-10">
            <div className="space-y-8">
              ...
              <div className="flex flex-wrap gap-4 pt-4">
                ...
                >
                  <a href="/generators/password-generator">
                ...
                <div className="p-3 bg-muted/50 border-t border-indigo-100 dark:border-indigo-800/20 flex items-center justify-between text-sm">
                  ...
                  <button
                    onClick={() => {
                      ...
                      clearSearch();
                      window.location.href = `/sitemap?q=${encodeURIComponent(q)}`;
                    }}
